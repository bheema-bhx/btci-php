...
{
    const VERSION           = '1.0.1';
    const USER_AGENT_ORIGIN = 'BtciPay PHP Library';
    ...
    public static function request($url, $method = 'POST', $params = array(), $authentication = array())
    {
        ...
        if (empty($app_id) || empty($app_key) || empty($app_secret))
            \BtciPay\Exception::throwException(400, array('reason' => 'CredentialsMissing'));
        ...
        if ($http_status === 200)
            ...
            \BtciPay\Exception::throwException($http_status, $response);
    }
}
